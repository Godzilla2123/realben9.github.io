...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		Game.Object=function(name,commonName,desc,icon,iconColumn,art,price,cps,buyFunction)
		{
			...
			this.sell=function(amount,bypass)
			{
				...
				if (success && Game.hasGod)
				{
					...
					if (old)
					{
						if (godLvl==1) old.multClick+=sold*0.03;
						else if (godLvl==2) old.multClick+=sold*0.02;
						else if (godLvl==3) old.multClick+=sold*0.01;
					}
					else
					{
						if (godLvl==1) Game.gainBuff('devastation',30,1+sold*0.03);
						else if (godLvl==2) Game.gainBuff('devastation',20,1+sold*0.02);
						else if (godLvl==3) Game.gainBuff('devastation',10,1+sold*0.01);
					}
				}
		...
		new Game.Object('Antimatter condenser','antimatter condenser|antimatter condensers|condensed|[X] extra quark per atom|[X] extra quarks per atom','Condenses the antimatter in the universe into cookies.',13,13,{base:'antimattercondenser',xV:0,yV:64,w:64,rows:1,x:0,y:0},3999999999,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
