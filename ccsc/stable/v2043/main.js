	...
	Game.Init=function()
	{
		...
		Game.UpdateAscendIntro=function()
		{
			...
			if (Game.AscendTimer>Game.AscendDuration)//end animation and launch ascend screen
			{
				...
				if (!App) PlaySound('snd/choir.mp3');
		...
		Game.sayTime=function(time,detail)
		{
			...
			else
			{
				...
				else */if (time>=Game.fps*60*60*24 && detail<2) str=loc("%1 day",Math.floor(time/(Game.fps*60*60*24)));//Beautify(Math.floor(time/(Game.fps*60*60*24)))+' days';
				else if (time>=Game.fps*60*60 && detail<3) str=loc("%1 hour",Math.floor(time/(Game.fps*60*60)));//Beautify(Math.floor(time/(Game.fps*60*60)))+' hours';
				else if (time>=Game.fps*60 && detail<4) str=loc("%1 minute",Math.floor(time/(Game.fps*60)));//Beautify(Math.floor(time/(Game.fps*60)))+' minutes';
				else if (time>=Game.fps && detail<5) str=loc("%1 second",Math.floor(time/(Game.fps)));//Beautify(Math.floor(time/(Game.fps)))+' seconds';
		...
		Game.Object=function(name,commonName,desc,icon,iconColumn,art,price,cps,buyFunction)
		{
			...
			else
			{
				...
				str+='<div class="productButtons">';
					str+='<div id="productLevel'+this.id+'" class="productButton productLevel lumpsOnly" onclick="Game.ObjectsById['+this.id+'].levelUp()" '+Game.getDynamicTooltip('Game.ObjectsById['+this.id+'].levelTooltip','this')+'></div>';
					str+='<div id="productMinigameButton'+this.id+'" class="productButton productMinigameButton lumpsOnly" onclick="Game.ObjectsById['+this.id+'].switchMinigame(-1);PlaySound(Game.ObjectsById['+this.id+'].onMinigame?\'snd/clickOn2.mp3\':\'snd/clickOff2.mp3\');"></div>';
					str+='<div class="productButton productMute" '+Game.getTooltip('<div style="width:150px;text-align:center;font-size:11px;"><b>'+loc("Mute")+'</b><br>('+loc("Minimize this building")+')</div>','this')+' onclick="Game.ObjectsById['+this.id+'].mute(1);PlaySound(Game.ObjectsById['+this.id+'].muted?\'snd/clickOff2.mp3\':\'snd/clickOn2.mp3\');" id="productMute'+this.id+'">'+loc("Mute")+'</div>';
				str+='</div>';
		...
		if (true)//if (!EN)
		{
			...
			for (var i in Game.Objects)
			{
				Game.foolObjects[i].name=loc(FindLocStringByPart(Game.Objects[i].name+' business name'));
				Game.foolObjects[i].desc=loc(FindLocStringByPart(Game.Objects[i].name+' business quote'));
			}
		}
