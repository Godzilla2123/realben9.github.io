...
M.launch=function()
{
	...
	M.init=function(div)
	{
		...
		M.spellTooltip=function(id)
		{
			return function(){
				...
				'<div class="line"></div><div class="description"><b>Effect :</b> <span class="green">'+me.desc+'</span>'+(me.failDesc?('<div style="height:8px;"></div><b>Backfire :</b> <span class="red">'+me.failDesc+'</span>'):'')+'</div></div>';
				return str;
			};
		}
		...
		AddEvent(M.lumpRefill,'click',function(){
			if (Game.lumps>=1 && M.magic<M.magicM && Game.canRefillLump())
			{
				Game.refillLump();
				M.magic+=100;
	...
	M.load=function(str)
	{
		...
		var on=parseInt(spl[i++]||0);if (on) M.parent.switchMinigame(1);
	}
	M.reset=function()
	{
		M.computeMagicM();
