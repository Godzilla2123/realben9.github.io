...
M.launch=function()
{
		...
		*/
		M.plants={
			...
			'cheapcap':{
				...
				mature:40,
				children:[],
				effsStr:'<div class="green">&bull; buildings and upgrades are 0.2% cheaper</div>',
		...
		M.getMuts=function(neighs,neighsM)
		{
			...
			if (neighsM['bakerWheat']>=1 && neighsM['gildmillet']>=1) muts.push(['clover',0.03],['goldenClover',0.0005]);
			if (neighsM['clover']>=1 && neighsM['gildmillet']>=1) muts.push(['shimmerlily',0.02]);
				if (neighsM['clover']>=2 && neighs['clover']<5) muts.push(['clover',0.005],['goldenClover',0.0001]);
				if (neighsM['clover']>=4) muts.push(['goldenClover',0.0005]);
			...
			if (neighsM['crumbspore']>=1 && neighsM['thumbcorn']>=1) muts.push(['glovemorel',0.01]);
			if (neighsM['crumbspore']>=1 && neighsM['shimmerlily']>=1) muts.push(['cheapcap',0.03]);
			if (neighsM['doughshroom']>=1 && neighsM['greenRot']>=1) muts.push(['foolBolete',0.02]);
			...
			if (neighsM['crumbspore']>=1 && neighsM['brownMold']>=1) muts.push(['wrinklegill',0.01]);
		...
		M.soils={
			...
			'clay':{
				...
				weedMult:1,
				req:100,
				effsStr:'<div class="gray">&bull; tick every <b>15 minutes</b></div><div class="green">&bull; passive plant effects <b>+50%</b></div>',
			...
			'pebbles':{
				...
				weedMult:0.1,
				req:200,
				effsStr:'<div class="gray">&bull; tick every <b>5 minutes</b></div><div class="red">&bull; passive plant effects <b>-75%</b></div><div class="green">&bull; <b>30% chance</b> of harvesting seeds automatically when plants expire</div><div class="green">&bull; weeds appear <b>10 times</b> less</div>',
		...
		M.tools={
			...
			'harvestAll':{
				...
				func:function(){PlaySound('snd/toneTick.mp3');/*if (M.freeze){return false;}*/M.harvestAll();},
			},
			'freeze':{
				name:'Freeze',
				icon:1,
				//desc:'Cryogenically preserve your garden.<br>Plants no longer grow, spread or die; they provide no benefits.<br>Nothing can be planted or harvested. Soil cannot be changed.<div class="line"></div>Using this will effectively pause your garden.',
				desc:'Cryogenically preserve your garden.<br>Plants no longer grow, spread or die; they provide no benefits.<br>Soil cannot be changed.<div class="line"></div>Using this will effectively pause your garden.',
				func:function(){
					PlaySound('snd/toneTick.mp3');
					M.freeze=(M.freeze?0:1);
					if (M.freeze) {M.computeEffs();PlaySound('snd/freezeGarden.mp3');this.classList.add('on');l('gardenContent').classList.add('gardenFrozen');}
					else {M.computeEffs();this.classList.remove('on');l('gardenContent').classList.remove('gardenFrozen');}
				},
		...
		M.seedTooltip=function(id)
		{
			return function(){
				...
				var str='<div style="padding:8px 4px;min-width:400px;">'+
					...
					(me.plantable?('<div style="float:right;text-align:right;width:100px;"><small>Planting cost :</small><br><span class="price'+(M.canPlant(me)?'':' disabled')+'">'+Beautify(Math.round(M.getCost(me)))+'</span><br><small>'+Game.sayTime(me.cost*60*30,-1)+' of CpS,<br>minimum '+Beautify(me.costM)+' cookies</small></div>'):'')+
			...
			return function(){
				...
				else
				{
					...
					var str='<div style="padding:8px 4px;min-width:350px;">'+
						...
						'<div style="text-align:center;">Click to harvest.</div>'+
						'<div class="line"></div>'+
						M.getPlantDesc(me)+
					'</div>';
					return str;
		...
		M.buildPanel=function()
		{
			...
			for (var i in M.soils)
			{
				...
				AddEvent(l('gardenSoil-'+me.id),'click',function(me){return function(){
					if (M.freeze || M.soil==me.id || M.nextSoil>Date.now() || M.parent.bought<me.req){return false;}
					PlaySound('snd/toneTick.mp3');
		...
		M.harvestAll=function(type)
		{
			...
			for (var i=0;i<2;i++)//we do it twice to take care of whatever spawns on kill
			{
				for (var y=0;y<6;y++)
				{
					for (var x=0;x<6;x++)
					{
						if (type && M.plot[y][x][0]>=1)
						{
							var tile=M.plot[y][x];
							var me=M.plantsById[tile[0]-1];
							if (me==type && tile[1]>=me.mature) harvested+=M.harvest(x,y)?1:0;
						}
						else harvested+=M.harvest(x,y)?1:0;
					}
				}
			}
		...
		'#gardenContent{position:relative;box-sizing:border-box;padding:4px 24px;height:'+(6*M.tileSize+16+48+32)+'px;}'+
		...
		str+='<div id="gardenDrag"><div id="gardenCursor" class="shadowFilter"></div></div>';
			...
			str+='<div id="gardenField">';
				...
				str+='<div style="margin-top:-12px;" id="gardenSoils"></div>';
		...
		AddEvent(M.lumpRefill,'click',function(){
			if (Game.lumps>=1 && Game.canRefillLump())
			{
				Game.refillLump();
				Game.lumps-=1;
				M.loopsMult=3;
	...
	M.load=function(str)
	{
		...
		var on=parseInt(spl2[i2++]||0);if (on) M.parent.switchMinigame(1);
						...
						if (M.isTileUnlocked(x,y))
						{
							...
							if (tile[0]>0)
							{
								...
								else if (tile[1]>=100)
								{
									...
									if (M.soilsById[M.soil].key=='pebbles' && Math.random()<0.3)
									{
	...
	M.draw=function()
	{
		...
		if (Game.drawT%10==0)
		{
			...
			{
				var me=M.soils[i];
				if (M.parent.bought<me.req) l('gardenSoil-'+me.id).classList.add('disabled');
