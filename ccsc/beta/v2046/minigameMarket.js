...
M.launch=function()
{
	...
	M.tick=function()
	{
		...
		for (var i=0;i<M.goodsById.length;i++)
		{
			...
			if (globD!=0 && Math.random()<globP) {me.val-=(1+me.d*Math.pow(Math.random(),3)*20)*globD;me.val-=globD*(1+Math.pow(Math.random(),3)*20);me.d+=globD*(1+Math.random()*6);me.dur=0;}
			
			me.val+=Math.pow((Math.random()-0.5)*2,11)*10;
