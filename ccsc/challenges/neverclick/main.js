...
function tinyIcon(icon,css)
{
	...
	return '<div class="icon" style="vertical-align:middle;display:inline-block;'+writeIcon(icon)+'transform:scale(0.5);margin:-16px;'+(css?css:'')+'"></div>';
}
...
(function(){
	...
	*/
	...
	Game.CheckModData=function()
	{
		...
		Game.Prompt('<h3>'+loc("Mod data")+'</h3><div class="block">'+tinyIcon([16,5])+'<div></div>'+loc("These are the mods present in your save data. You may delete some of this data to make your save file smaller.")+'</div><div class="block" style="font-size:11px;">'+str+'</div>',[loc("Back")]);
	}
...
Game.Launch=function()
{
	...
	'<div class="listing">Miss the old version? Your old save was automatically exported <a href="//realben9.github.io/ccsc/legacy/v10466" target="_blank">here</a>!</div>'+
	...
	'<div class="listing">&bull; the dungeons beta is now available on <a href="//realben9.github.io/ccsc/legacy/v1037" target="_blank">/betadungeons</a></div>'+
	...
	'<div class="listing">&bull; dungeons still need a bit more work and will be released soon - we want them to be just right! (you can test an unfinished version in <a href="//realben9.github.io/ccsc/legacy/v1037/" target="_blank">the beta</a>)</div>'+
	
	'</div><div class="subsection update">'+
	'<div class="title">28/09/2013 - dungeon beta</div>'+
	'<div class="listing">&bull; from now on, big updates will come through a beta stage first (you can <a href="//realben9.github.io/ccsc/legacy/v1037/" target="_blank">try it here</a>)</div>'+
	...
	Game.Init=function()
	{
		...
		Game.cookiesEarned=15;//all cookies earned during gameplay
		Game.cookies=15;//cookies
		Game.cookiesd=15;//cookies display
		...
		Game.cookieClicks=15;//+1 for each click on the cookie
		...
		Game.handmadeCookies=15;//all the cookies made from clicking the cookie
		...
		Game.Reset=function(hard)
		{
			...
			if (hard) {Game.clicksThisSession=0;Game.jukebox.reset();PlayCue('launch');}
			else PlayCue('play');
		}
		Game.HardReset=function(bypass)
		{
			if (!bypass)
			{
				Game.Prompt('<h3>'+loc("Wipe save")+'</h3><div class="block">'+tinyIcon([15,5])+'<div class="line"></div>'+loc("Do you REALLY want to wipe your save?<br><small>You will lose your progress, your achievements, and your heavenly chips!</small>")+'</div>',[[EN?'Yes!':loc("Yes"),'Game.ClosePrompt();Game.HardReset(1);','float:left'],[loc("No"),0,'float:right']]);
			}
			else if (bypass==1)
			{
				Game.Prompt('<h3>'+loc("Wipe save")+'</h3><div class="block">'+tinyIcon([15,5])+'<div class="line"></div>'+loc("Whoah now, are you really, <b><i>REALLY</i></b> sure you want to go through with this?<br><small>Don't say we didn't warn you!</small>")+'</div>',[[EN?'Do it!':loc("Yes"),'Game.ClosePrompt();Game.HardReset(2);','float:left'],[loc("No"),0,'float:right']]);
			}
			else
			{
				for (var i in Game.AchievementsById)
				...
				Game.cookiesd=15;//cookies display
				Game.cookieClicks=15;//+1 for each click on the cookie
				Game.handmadeCookies=15;//all the cookies made from clicking the cookieGame.AchievementsOwned=0;
				Game.goldenClicks=0;
				Game.missedGoldenClicks=0;
				Game.Reset(1);
				Game.resets=0;
		...
		{
			if (!bypass) Game.Prompt('<h3>'+loc("Ascend")+'</h3><div class="block">'+tinyIcon([19,7])+'<div class="line"></div>'+loc("Do you REALLY want to ascend?<div class=\"line\"></div>You will lose your progress and start over from scratch.<div class=\"line\"></div>All your cookies will be converted into prestige and heavenly chips.")+'<div class="line"></div>'+(Game.canLumps()?loc("You will keep your achievements, building levels and sugar lumps."):loc("You will keep your achievements."))+'</div>',[[EN?'Yes!':loc("Yes"),'Game.ClosePrompt();Game.Ascend(1);','float:left'],[loc("No"),0,'float:right']]);
			else
			{
		...
		Game.Object=function(name,commonName,desc,icon,iconColumn,art,price,cps,buyFunction)
		{
			...
			this.tooltip=function()
			{
				...
				return '<div style="position:absolute;left:1px;top:1px;right:1px;bottom:1px;background:linear-gradient(125deg,'+(false?'rgba(15,115,130,1) 0%,rgba(15,115,130,0)':'rgba(50,40,40,1) 0%,rgba(50,40,40,0)')+' 20%);mix-blend-mode:screen;z-index:1;"></div><div style="z-index:10;min-width:350px;padding:8px;position:relative;"><div class="icon" style="float:left;margin-left:-8px;margin-top:-8px;'+writeIcon(icon)+'"></div><div style="float:right;text-align:right;"><span class="price'+(canBuy?'':' disabled')+'">'+Beautify(Math.round(price))+'</span>'+Game.costDetails(price)+'</div><div class="name">'+name+'</div>'+'<small><div class="tag">'+loc("owned: %1",me.amount)+'</div>'+(me.free>0?'<div class="tag">'+loc("free: %1!",me.free)+'</div>':'')+'</small>'+
	...
	Game.Logic=function()
	{
		...
		if (!Game.OnAscend && Game.AscendTimer==0)
		{
			...
			{
				if (isNaN(Game.cookies)) {Game.cookies=15;Game.cookiesEarned=15;Game.recalculateGains=1;}
